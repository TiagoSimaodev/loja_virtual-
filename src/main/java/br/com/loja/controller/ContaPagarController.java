...
import br.com.loja.model.ContaPagar;
import br.com.loja.repository.ContaPagarRepository;
...
public class ContaPagarController {
	...
	public ResponseEntity<ContaPagar> salvarContaPagar(@RequestBody @Valid ContaPagar contaPagar) throws ExceptionLojaVirtual {//Recebe o json e converte para objeto
		...
		if(contaPagar.getPessoa_fornecedor() == null || contaPagar.getPessoa_fornecedor().getId() <= 0) {
			throw new ExceptionLojaVirtual("Fornecedor responsável deve ser informada");
		...
		if (contaPagar.getId() == null) {
			...
			if(!contaPagars.isEmpty()) {
				throw new ExceptionLojaVirtual("Já existe conta a pagar com a mesma descrição");
	...
	public ResponseEntity<ContaPagar> obterContaPagar(@PathVariable("id") Long id) throws ExceptionLojaVirtual{
		...
		if(contaPagar == null) {
			throw new ExceptionLojaVirtual("Não encontrou Conta a Pagar  com código: " + id);
		}
